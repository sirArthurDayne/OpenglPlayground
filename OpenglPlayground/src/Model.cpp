...
void Model::ProcessNodes(const aiNode* node, const aiScene* scene)
{
	...
	for (unsigned int i = 0; i < node->mNumMeshes; i++)
	{
		...
		auto [vertices,indices,texturesData, materialData] = ProcessMesh(mesh, scene);
		m_meshLoaded.emplace_back(vertices,indices,texturesData, materialData);
	}
	...
	Model::ProcessMesh(const aiMesh* mesh, const aiScene* scene)
{
	...
	if (mesh->mMaterialIndex >= 0)
	{
		...
		if (m_hasMaterials)
		{
			...
			bool exits = false;
			for (auto& mat : m_materialsLoaded)
			{
				if (name == mat.name)
				{
					exits = true;
				...
				auto makeMaterial = [=] (aiColor4D amb, aiColor4D dif, aiColor4D spec, float sh)
			{
				...
				return Material(name, ambient, difuse, specular, sh);
			};

				Material mat = makeMaterial(ambientColor, diffuseColor, specularColor, shinniness);
				out_materials.push_back(mat);
				m_materialsLoaded.push_back(mat);
				std::cout << "Material Add: " << material->GetName().C_Str() << std::endl;
			}
		}
	...
	//make tuple
	return {out_vertices,out_indices,out_textures,out_materials};
}
